...
export default function App() {
  ...
  const [googleSheetSource, setGoogleSheetSource] = useState<string>('');
  const [activeTab, setActiveTab] = useState<'overview' | 'executive-briefing' | 'data-entry' | 'whole-data' | 'trends' | 'cd1-driver-tree' | 'issue-dashboard'>('overview');
    ...
    <div className="min-h-screen bg-slate-50/50 flex flex-col font-sans text-slate-800">
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3.5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          {/* Far Right: Dashboard Last Updated & Google Sheets Live Sync */}
          <div className="flex flex-wrap items-center gap-2 sm:gap-3 self-end sm:self-auto">
            <button
              onClick={() => setIsGoogleSheetsModalOpen(true)}
              className="bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-300 rounded-xl px-3.5 py-1.5 flex items-center gap-2 shadow-2xs text-xs font-bold transition-all cursor-pointer hover:shadow-sm active:translate-y-0"
            >
              <Cloud className="w-4 h-4 text-emerald-600 shrink-0" />
              <span>Google Sheets Sync</span>
              <span className="bg-emerald-200 text-emerald-900 text-[10px] px-1.5 py-0.5 rounded-full font-extrabold">LIVE</span>
            </button>
      ...
      <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-6 flex-1 flex flex-col gap-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-2">
          ...
          <nav className="flex overflow-x-auto no-scrollbar max-w-full gap-2 bg-slate-200/70 p-1.5 rounded-xl border border-slate-300/60 shadow-xs self-start">
            ...
            {/* Tab 1.5: Executive Briefing (v2 Slot) */}
            <button
              onClick={() => {
                setActiveTab('executive-briefing');
                setEditingTicket(null);
              }}
              className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-xs font-bold transition-all duration-200 cursor-pointer select-none ${
            ...
            >
              <Sparkles className={`w-4 h-4 ${activeTab === 'executive-briefing' ? 'text-yellow-300' : 'text-indigo-600'}`} />
              Executive Briefing <span className="bg-amber-100 text-amber-900 text-[10px] px-1.5 py-0.5 rounded font-extrabold">v2</span>
            </button>
        ...
        <main className="flex-1">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {activeTab === 'executive-briefing' && (
                <ExecutiveBriefingView tickets={tickets} issues={issues} />
              )}
      ...
      <GoogleSheetsSyncModal
        ...
        currentSheetUrl={googleSheetSource}
        onDataLoaded={(loadedTickets, sourceUrl) => {
          saveTicketsState(loadedTickets);
          setGoogleSheetSource(sourceUrl);
          const todayStr = '07 Aug 2026';
          setLastRiskImportDate(todayStr);
          localStorage.setItem(LOCAL_STORAGE_LAST_RISK_IMPORT_KEY, todayStr);
        }}
      />
