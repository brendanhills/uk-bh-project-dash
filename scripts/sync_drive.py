...
def ensure_latest_podcast_generated(
    ...
    audio_asset_path = os.path.join(PARENT_DIR, 'assets', f"podcast_w{max_week}.mp3") if project_name == 'monaro' else os.path.join(proj_dir, f"podcast_w{max_week}.mp3")
    proj_audio_path = os.path.join(proj_dir, f"podcast_w{max_week}.mp3")
    ...
    logger.info(f"Generating Gemini 3.5 Flash podcast and Chirp 3 HD audio for latest snapshot '{latest_key}' (week {max_week})...")
    ...
    if new_script:
        ...
        resolved_audio_file = f"assets/podcast_w{max_week}.mp3" if project_name == 'monaro' else f"data/{project_name}/podcast_w{max_week}.mp3"
        ...
        try:
            with open(snapshots_path, 'rb') as f:
                snap_bytes = f.read()
            upload_bytes_to_gcs(snap_bytes, target_bucket, f"{project_name}/snapshots.json", content_type="application/json")
        except Exception as e:
