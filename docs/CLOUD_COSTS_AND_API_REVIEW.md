...
The environment developer cockpit ([`setup.sh`](../setup.sh)) and Terraform API module (`deploy/terraform/modules/apis/`) manage all required Google Cloud APIs. Below is the operational justification and architectural role for each service:
...
### 1. Browser Caching for Application Code
- **Current State**: [`server.py`](../server.py) serves static assets (`/assets/`, `/src/`) and sets strict no-cache headers for dynamic datasets (`/data/`, `snapshots.json`, and API responses).
- **Benefit**: Eliminates repeated asset transfers while preserving instant real-time data freshness on new weekly report ingestion.
